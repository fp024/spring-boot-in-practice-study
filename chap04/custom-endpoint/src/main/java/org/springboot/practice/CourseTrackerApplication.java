...
import org.springboot.practice.model.ReleaseItem;
...
public class CourseTrackerApplication {
  ...
  Collection<ReleaseNote> loadReleaseNotes() {
        ...
        ReleaseNote.builder()
            ...
            .bugFixes(
                Set.of(
                    getReleaseItem(
                        "SBIP-123",
                        "The name of the matching-strategy property is incorrect in the action"
                            + " message of the failure analysis for a PatternParseException"
                            + " #28839"),
                    getReleaseItem(
                        "SBIP-124",
                        "ErrorPageSecurityFilter prevents deployment to a Servlet 3.1 compatible"
                            + " container #28790")))
            .build();

    ReleaseNote releaseNote2 =
        ReleaseNote.builder()
            .version("v1.2.0")
  ...
  private ReleaseItem getReleaseItem(String itemId, String itemDescription) {
    return ReleaseItem.builder().itemId(itemId).itemDescription(itemDescription).build();
  }
}
