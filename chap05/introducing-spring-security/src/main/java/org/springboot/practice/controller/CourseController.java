...
public class CourseController {
  ...
  public String index(Model model) {
    ...
    model.addAttribute("courses", courseList.isEmpty() ? Collections.emptyList() : courseList);
    return "index";
  }
  ...
  public String showUpdateCourseForm(@PathVariable("id") Long id, Model model) {
    courseService.findCourseById(id).ifPresent(course -> model.addAttribute("course", course));
    return "update-course";
  }

  @PutMapping("/update/{id}")
  public String updateCourse(
      @PathVariable("id") Long id, @Valid Course course, BindingResult result, Model model) {
    if (result.hasErrors()) {
      course.setId(id);
      return "update-course";
    }
